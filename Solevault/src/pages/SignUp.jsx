...
const SignUp = () => {
    ...
    try {
      ...
      if (response.data && response.data.token && response.data.user) {
        ...
        toast({
          ...
          description: `Welcome, ${response.data.user.firstName}! Your account has been created successfully.`,
        });
        
        // Navigate to dashboard since user is now also logged in
        navigate("/dashboard"); 
      } else {
        // Fallback if token/user is not in response, though backend should send it
        toast({
  ...
  return (
    // JSX for your form remains the same
      ...
      <div className="bg-[#262626] border border-[#404040] rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 w-full max-w-md p-8">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            {/* Your FormField components for firstName, lastName, email, password, confirmPassword */}
